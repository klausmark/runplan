...
class WeekSelectionTests(unittest.TestCase):
    ...
    def test_all_selects_every_available_week(self) -> None:
        self.assertEqual(
            (1, 2, 3), WeekSelection.all().resolve((1, 2, 3))
        )
    ...
    def test_current_and_next_use_an_injected_date(self) -> None:
        ...
        self.assertEqual(
            (3,),
            WeekSelection.current().resolve(
                range(1, 6), start_date=start, today=date(2026, 7, 20)
            ),
        )
        self.assertEqual(
            (4,),
    ...
    def test_weeks_ahead_selects_current_and_subsequent_complete_plan_weeks(self) -> None:
        ...
        self.assertEqual(
            (1, 2),
            WeekSelection.ahead(1).resolve(
                range(1, 5), start_date=start, today=date(2026, 7, 26)
            ),
        )
        self.assertEqual(
            (2, 3),
            WeekSelection.ahead(1).resolve(
                range(1, 5), start_date=start, today=date(2026, 7, 27)
            ),
        )
        self.assertEqual(
            (4,),
            WeekSelection.ahead(2).resolve(
                range(1, 5), start_date=start, today=date(2026, 8, 10)
            ),
        )
    ...
    def test_relative_selection_requires_dates_and_must_be_in_program(self) -> None:
        ...
        for outside in (date(2026, 6, 29), date(2026, 7, 20)):
            with self.subTest(outside=outside), self.assertRaisesRegex(
                WeekSelectionError, "current plan week.*outside"
            ):
                WeekSelection.ahead(1).resolve(
                    (1, 2), start_date=date(2026, 7, 6), today=outside
                )
